...
# 출력 파일 경로 설정
output_file = 'V1__insert_words.sql'

# word_id를 추적하기 위한 딕셔너리
word_ids = {}
...
with open(csv_file_path, 'r', encoding='utf-8') as csvfile, \
    ...
    # SQL 헤더 작성
    sqlfile.write('-- Insert words into the word table\n')
    sqlfile.write('INSERT INTO word (word, is_phrasal_verb) VALUES\n')
    ...
    for i, row in enumerate(rows[:-1]):
        word = row['word'].replace("'", "''")  # SQL 문자열 이스케이프
        sqlfile.write(f"('{word}', 1),\n")
    
    # 마지막 행은 콤마 없이 작성
    if rows:
        last_word = rows[-1]['word'].replace("'", "''")
